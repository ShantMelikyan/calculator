  ...
  element.addEventListener("click", () => {
    ...
    if (index == 0) {
     ...
     // document.querySelectorAll('.tri-state-toggle input').style.backgroundColor = '';
      ...
      document.getElementsByTagName("body")[0].style.backgroundColor = "hsl(240, 75%, 9%)"; // done
      document.querySelectorAll('.btn').forEach(element => element.style.backgroundColor = 'hsl(268, 47%, 21%)');
      document.querySelectorAll('.btn').forEach(element => element.style.borderBottom = '3px solid #871c9c'); //done
      document.getElementsByTagName("body")[0].style.color = 'hsl(52, 100%, 62%)'; //done
      document.querySelector('.tri-state-toggle').style.backgroundColor = 'hsl(268, 75%, 9%)';
      document.querySelector('.result').style.backgroundColor = 'hsl(268, 75%, 9%)'; // doone
      document.querySelector('.result').style.color ="hsl(52, 100%, 62%)";
      document.querySelector('.clear').style.backgroundColor = "hsl(290, 70%, 36%)"; //done
      document.querySelector('.delete').style.backgroundColor = 'hsl(290, 70%, 36%)'; //done
      ...
      document.querySelector('.calculate').style.borderBottom = ('3px solid hsl(177, 92%, 70%)'); //done
      document.querySelector('.calculate').style.backgroundColor = 'hsl(176, 100%, 44%)'; //dome
      document.querySelector('.buttons').style.backgroundColor = 'hsl(268, 75%, 9%)'; // done
      document.querySelectorAll('.btn').forEach(element => element.style.color = 'hsl(52, 100%, 62%)'); //done text color
      const elements = document.querySelectorAll('.tri-state-toggle input');
      for (let i = 0; i < elements.length; i++) {
